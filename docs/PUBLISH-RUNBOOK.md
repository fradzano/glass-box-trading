...
The route the submission cites, `/revisions/sha256-7b82959a344a7c7e/presentation/`, is byte-stable through the whole step: SHA-256 prefix `c8745e3f5dc00401` over 157,652 bytes on the live alias before the render, on the candidate, and on the alias after the promotion, equal to the local file in `out\deploy`. Its `projection.json` still names `sha256:7b82959a344a7c7e` at cutoff `2026-09-03T20:00:14.787Z`, last seq 76.

**Both probe runs came back 47 of 48, and the red line was the instrument, not the site.** The manifest's `jsonRoutes` was every `.json` in the deploy tree, and the probe expected each of them to name the manifest's current `journalRevision`; a carried-forward immutable route must name its own, older revision, so `/revisions/sha256-7b82959a344a7c7e/presentation/projection.json` failed by construction. Earlier snapshots stayed green only because no carried-forward route had a `projection.json` yet. Everything that check protects was measured by hand instead (the paragraph above), and the promotion was ruled on that basis (DECISIONS 2026-09-04); `vercel rollback` remained the way back.

**Fixed 2026-09-05.** `jsonRoutes` is now a list of `{ url, expectedJournalRevision }`: the current revision for a route this render wrote, the route's own older revision for a carried-forward immutable one, and `null` for a spelling this project cannot produce — which the probe fails loudly rather than passing on a guess. `tests/publish-dashboard.spec.ts` pins both the derivation and the carried-forward shape end to end (render a shorter journal, render the grown one into the same deploy tree, assert every stated expectation against the bytes on disk). A failing probe now also names its failed checks in the summary line instead of only counting them, so step 3's "a failed probe means the candidate is not promoted" no longer sends the operator scrolling. A manifest produced by an older render has no `url` field per entry; the probe says so and fails, rather than probing nothing.
