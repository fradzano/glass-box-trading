...
| ID | SPEC case(s) | Trigger path to execute |
|---|---|---|
| WIN-1 | S-CYC-06, S-J-09 | journal-only failure with open exposure → deterministic risk-reducing emergency close, explicit audit-gap reconciliation, no invented INTENT; authority-store failure → no mutation ◐ P3 part: `tests/cyc-runner.spec.ts` S-CYC-06 (journal read-only + kill → deterministic risk-reducing close through the gateway without INTENT; next writable cycle appends `RECONCILIATION` `AUDIT_GAP_EMERGENCY_CLOSE` with broker data and the explicit "no durable prior INTENT" statement; unreadable epoch store → no mutation at all); the S-J-09 projection link is P6 — row stays open for that part |
