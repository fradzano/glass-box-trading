...
- **Backlog from the close session (B, DECISIONS 2026-09-04):** the publish
  manifest must state an expected journal revision per JSON route — for a
  carried-forward immutable route the one spelled in its own path, not the
  current one — and `tests/publish-dashboard.spec.ts` must pin it. Until
  then a red `probe-dashboard.ps1` line on such a route is expected, and
  the runbook's "a failed probe means the candidate is not promoted"
  requires the operator to read which check failed.
