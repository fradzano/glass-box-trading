...
/**
 * The digest bound into `runtimeDigest` as `launchArtifactsSha256`: every file
 * of the importable site tree, at any depth, as `<relative path> <sha256>`
 * lines sorted and hashed. Paths are relative to the site root, so the same
 * tree installed elsewhere digests identically; a rename, a new file, or a
 * single changed byte anywhere in the tree changes it.
 *
 * The ONLY exclusions are the generated bytecode the launcher deletes and
 * re-verifies as absent before every spawn — `__pycache__` directories at any
 * depth and `.pyc` files — because those are recreated by the interpreter and
 * would make the digest of an unchanged environment unstable. Nothing else is
 * skipped: unlike `dependencySiteSha256`, which deliberately narrows to the
 * canonical dependency bytes, this digest also covers `bin/`, `.lock`,
 * installer metadata and every dotfile.
 */
export async function launchArtifactsDigest(site: string, deadlineMs: number): Promise<string> {
  const siteFiles: string[] = [];
  await walkFilesAsync(site, siteFiles, deadlineMs);
  ...
  return createHash("sha256").update(artifactLines.sort().join("\n")).digest("hex");
}
...
export function createEnvironmentPorts(paths: AnalystEnvironmentPaths, packageModuleDir = "alpaca_mcp_server"): { readonly evidence: McpEvidencePort; readonly child: McpChildPort; readonly extra: () => EnvironmentEvidence; readonly lastObservation: () => LaunchObservation | null } {
  ...
  const evidence: McpEvidencePort = {
    async gather(lock: RuntimeLock, operationTimeoutMs: number): Promise<Omit<McpLaunchObservation, "childEnvironment">> {
      ...
      launchArtifactsSha256 = await launchArtifactsDigest(paths.site, deadlineMs);
