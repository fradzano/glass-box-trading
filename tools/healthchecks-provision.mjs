...
//   node tools/healthchecks-provision.mjs --pause
...
//
// Every exit goes through the return value of main(), never process.exit().
// Calling process.exit() while fetch's keep-alive sockets are still closing
// trips a libuv assertion on Windows and turns a finished `--list` into exit
// code 9 -- harmless to a reader, and a false failure to any script that reads
// the exit code.
...
function parseArgs(argv) {
  const options = { list: false, pause: false, rotate: false, apply: false, tz: null, cycleCron: null, watchdogCron: null };
  for (let i = 0; i < argv.length; i += 1) {
    const flag = argv[i];
    if (flag === "--list") options.list = true;
    else if (flag === "--pause") options.pause = true;
...
async function main() {
  ...
  // Pause exactly the three checks whose ping URLs are in .env -- matched by
  // URL, not by name, because names are not unique on healthchecks.io and the
  // URL is what the deployment actually pings. A paused check does not alarm on
  // silence, and the next ping resumes it by itself.
  if (options.pause) {
    const targets = [];
    for (const check of CHECKS) {
      ...
      if (match === undefined) {
        process.stderr.write(`${check.name}: no check in this project holds the URL in ${check.env}; nothing was paused.\n`);
        return 1;
      }
      targets.push({ name: check.name, check: match });
    }
    for (const { name, check } of targets) {
      await api(key, "POST", String(check.pause_url ?? `${API}/checks/${String(check.uuid)}/pause`));
      const after = await api(key, "GET", String(check.update_url ?? `${API}/checks/${String(check.uuid)}`));
      process.stdout.write(`paused    ${name.padEnd(16)} ${fingerprint(check.ping_url)}  status=${String(after?.status ?? "?")}\n`);
    }
    return 0;
  }
