...
import { launchArtifactsDigest } from "../src/shell/mcp-environment.js";
...
const NO_COUPLING = { declaredLongRun: { kind: "key", value: "declared-long-run" }, dotEnvRead: "parsed", duplicateKeys: [], dotEnvProfile: "dev", dotEnvStateDir: { kind: "none" }, effectiveStateDir: { kind: "none" } } as const;
...
// The digest bound into `runtimeDigest` as `launchArtifactsSha256`. Its claim is
// coverage of the COMPLETE importable site tree of the pinned analyst
// environment — the pinned package gets the stronger git-object comparison, but
// a dependency that keeps its locked version metadata while its installed bytes
// change is only caught here. Nothing tested that the walk actually reaches the
// bottom of the tree, so these tests plant changes three directories deep and
// pin the documented skip set: `__pycache__` directories and `.pyc` files, and
// nothing else (in particular not dotfiles, which the narrower
// `dependencySiteSha256` does exclude for `.lock`).
describe("P7 launch hardening — the MCP launch-artifact digest covers the whole site tree", () => {
  const BUDGET_MS = 30_000;
  const digestOf = (root: string): Promise<string> => launchArtifactsDigest(root, Date.now() + BUDGET_MS);

  function plantTree(root: string): void {
    mkdirSync(path.join(root, "pkg", "sub", "deeper"), { recursive: true });
    writeFileSync(path.join(root, "top.py"), "top\n", "utf8");
    writeFileSync(path.join(root, "pkg", "__init__.py"), "one\n", "utf8");
    writeFileSync(path.join(root, "pkg", "sub", "mid.py"), "two\n", "utf8");
    writeFileSync(path.join(root, "pkg", "sub", "deeper", "leaf.py"), "three\n", "utf8");
  }

  it("changes when a file appears three directories deep", async () => {
    const site = temporaryDirectory("gbt-p7-artifacts-add-");
    plantTree(site);
    const before = await digestOf(site);
    writeFileSync(path.join(site, "pkg", "sub", "deeper", "planted.py"), "", "utf8");
    expect(await digestOf(site)).not.toBe(before);
  });

  it("changes when a single byte of a deep file changes", async () => {
    const site = temporaryDirectory("gbt-p7-artifacts-byte-");
    plantTree(site);
    const before = await digestOf(site);
    writeFileSync(path.join(site, "pkg", "sub", "deeper", "leaf.py"), "thref\n", "utf8");
    expect(await digestOf(site)).not.toBe(before);
  });

  it("changes when a deep file is renamed even though every byte survives", async () => {
    const site = temporaryDirectory("gbt-p7-artifacts-rename-");
    plantTree(site);
    const before = await digestOf(site);
    const deep = path.join(site, "pkg", "sub", "deeper");
    renameSync(path.join(deep, "leaf.py"), path.join(deep, "leaf_renamed.py"));
    expect(await digestOf(site)).not.toBe(before);
  });

  it("hashes paths relative to the site root, so an identical tree elsewhere has the same digest", async () => {
    const here = temporaryDirectory("gbt-p7-artifacts-here-");
    const there = temporaryDirectory("gbt-p7-artifacts-there-");
    plantTree(here);
    plantTree(there);
    expect(await digestOf(there)).toBe(await digestOf(here));
  });

  it("skips exactly the generated bytecode set: __pycache__ directories and .pyc files, nothing else", async () => {
    const site = temporaryDirectory("gbt-p7-artifacts-skip-");
    plantTree(site);
    const before = await digestOf(site);

    const deep = path.join(site, "pkg", "sub", "deeper");
    mkdirSync(path.join(deep, "__pycache__"), { recursive: true });
    writeFileSync(path.join(deep, "__pycache__", "leaf.cpython-314.pyc"), "bytecode", "utf8");
    writeFileSync(path.join(deep, "leaf.pyc"), "bytecode", "utf8");
    mkdirSync(path.join(site, "__pycache__"), { recursive: true });
    writeFileSync(path.join(site, "__pycache__", "top.cpython-314.pyc"), "bytecode", "utf8");
    expect(await digestOf(site)).toBe(before);

    // Everything else at the same depth counts, dotfiles and non-Python files included.
    writeFileSync(path.join(deep, ".hidden"), "x", "utf8");
    const withDotfile = await digestOf(site);
    expect(withDotfile).not.toBe(before);
    writeFileSync(path.join(deep, "notes.txt"), "x", "utf8");
    expect(await digestOf(site)).not.toBe(withDotfile);
  });
});
