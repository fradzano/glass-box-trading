...
describe("activation fold — attempts and their steps", () => {
  ...
  it("keeps the latest earlier preflight as the baseline a new attempt's step 0 is compared against", () => {
    const text = ledger([
      ...
      { attempt: "a1", anchorDay: MON, kind: "result", step: "0-preflight", outcome: "ok", evidence: { wrapperHashes: { "cycle-run.ps1": "w1", "watchdog-run.ps1": "w2", "run-log.psm1": "w3", "watchdog-bootstrap.psm1": "w4" } } },
