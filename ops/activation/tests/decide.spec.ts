...
function happy(step: StepId): { readonly at: Clock; readonly evidence: Readonly<Record<string, unknown>> } {
  switch (step) {
    case "0-preflight": return { at: [MON, 15, 30], evidence: { wrapperHashes: { "cycle-run.ps1": "w1", "watchdog-run.ps1": "w2", "run-log.psm1": "w3", "watchdog-bootstrap.psm1": "w4" }, hostPreconditions: HOST } };
...
describe("decide — 0-resume judges the world against the phase", () => {
  ...
  it("aborts when either wrapper's hash changed since step 0, naming the wrapper (review 2026-09-14, point 6)", () => {
    const fold = before("4-enable");
    const watchdogEdited = decide(fold, worldFor(fold, [MON, 22, 6], { wrapperHashes: known({ "cycle-run.ps1": "w1", "watchdog-run.ps1": "w2-edited", "run-log.psm1": "w3", "watchdog-bootstrap.psm1": "w4" }) }), SCHEDULE);
    expect(evidenceOf(watchdogEdited)["red"]).toEqual(["wrapper.watchdog-run.ps1.sha256-changed-since-step-0"]);
    const cycleEdited = decide(fold, worldFor(fold, [MON, 22, 6], { wrapperHashes: known({ "cycle-run.ps1": "w1-edited", "watchdog-run.ps1": "w2", "run-log.psm1": "w3", "watchdog-bootstrap.psm1": "w4" }) }), SCHEDULE);
...
describe("decide — step 0, preflight", () => {
  ...
  it("removes the stale certificate line and records the wrapper hash and host preconditions", () => {
    ...
    expect(evidenceOf(decision)).toMatchObject({ wrapperHashes: { "cycle-run.ps1": "w1", "watchdog-run.ps1": "w2", "run-log.psm1": "w3", "watchdog-bootstrap.psm1": "w4" }, hostPreconditions: HOST, envHashBefore: "e0", fingerprints: FINGERPRINTS, tokenProbe: "ok" });
...
describe("decide — a new attempt inherits no confirmation age and no flat check (review 2026-09-14, point 3)", () => {
  ...
  it("keeps the wrapper baseline across attempts: a wrapper changed since the previous attempt's step 0 is red at the new step 0", () => {
    const fold = foldOf([...firstAttempt, opened([NEXT_MON, 15, 0], "a2", NEXT_TUE)]);
    const decision = decide(fold, worldFor(fold, [NEXT_MON, 15, 30], { wrapperHashes: known({ "cycle-run.ps1": "w1-edited", "watchdog-run.ps1": "w2", "run-log.psm1": "w3", "watchdog-bootstrap.psm1": "w4" }) }), scheduleFor(NEXT_MON, NEXT_TUE));
