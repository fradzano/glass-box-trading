...
| Long description ≥100 words, matches `submission/COPY.md` exactly | Submission form | Open form, read rendered field, word-count and diff against COPY.md | COPY.md final: 308 words, 1980 characters (form limit 600–2000); form-side paste is the owner step | 2026-09-04T11:58:00Z |
...
| Video upload: MP4, under five minutes, under 300 MB | Submission form file upload | Download the uploaded file back; check duration and file size | Local pre-upload facts (ffprobe, narrated second cut): `submission/glass-box-trading.mp4`, h264 1920x1080 30 fps + AAC 48 kHz voice-over, duration 299.03 s (< 300 s), 48,268,634 bytes (46.0 MB < 300 MB); rendered by `npm run render` in `video/` from the frozen dataset, five captures and nine narration files named in meta.json; archived in the verification store `evidence/submission-2026-09-04/`. Form-side half: owner step at form time | 2026-09-04T10:45:00Z |
...
| Repository URL resolves, exact submitted revision, anonymous access | `submission/COPY.md` / form URL field | Open URL in clean browser with no auth; confirm commit SHA matches the intended submitted revision | PASS: public, anonymous 200; the owner merged `p7/dev-live-certificate` `--no-ff` into `main` (merge commit `1951a44`, pushed 2026-09-04 11:25Z) from a separate worktree, so the plain repository URL shows the submitted state and its README opens with the submission section; submitted revision = `main` at the form instant (record the SHA in the form-side half) | 2026-09-04T11:27:00Z |
